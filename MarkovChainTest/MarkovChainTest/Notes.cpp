	...
	if (evt->isNoteOn())
	{
		for (int i = 0; i < 12; i++)
		{
			...
			if (localNote.isValid)
			{
				...
				if (evt->isLinked())
				{
					newNote.duration = evt->getTickDuration();
				}
...
int Notes::getOctave(const Note newNote, const int noteVal)
{
	int val = 0;
	if (newNote.cNote != "Err")
	{
		auto submap = noteMidiValues.at(newNote.cNote);
		for (int i = 0; i < submap.size(); i++)
		{
			if (submap.at(i) == noteVal)
			{
				val = i;
				break;
			}
...
Note::Note(MidiEvent * evt)
{
	Notes notes;
	Note locNote = notes.getNoteType(evt);
	this->cNote = locNote.cNote;
	this->duration = locNote.duration;
	this->isValid = locNote.isValid;
	this->octave = locNote.octave;
	this->velocity = locNote.velocity;
}
